...
public class day73_articulation_points {
    static int time=0;
    static void dfs(int u,int parent,List<List<Integer>> adj,int[] disc,int[] low,boolean[] ap){
        disc[u]=low[u]=++time;
        int children=0;
        for(int v: adj.get(u)){
            if(disc[v]==-1){
    ...
    public static void main(String[] args){
        ...
        for(int i=0;i<n;i++) if(disc[i]==-1) dfs(i,-1,adj,disc,low,ap);

        System.out.println("Articulation Points:");
        for(int i=0;i<n;i++) if(ap[i]) System.out.println(i);
    }
}
