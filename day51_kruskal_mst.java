...
public class day51_kruskal_mst {
    int V, E;
    Edge[] edges;

    day51_kruskal_mst(int v, int e) {
        V = v; E = e;
        edges = new Edge[E];
    ...
    void KruskalMST() {
        Edge[] result = new Edge[V]; int e = 0, i = 0;
        for (i = 0; i < V; ++i) result[i] = new Edge();
        Arrays.sort(edges);
        Subset[] subsets = new Subset[V];
        for (i = 0; i < V; ++i) {
            subsets[i] = new Subset(); subsets[i].parent = i; subsets[i].rank = 0;
        }
        i = 0;
        while (e < V - 1) {
    ...
    public static void main(String[] args) {
        int V = 4, E = 5;
        day51_kruskal_mst graph = new day51_kruskal_mst(V, E);
